...
*/
package com.tengos.sociot.api.authorizer.io;
 ...
 * AWS policy for the API Gateway authorizer. The constructor receives the
 * calling user principal, the AWS account ID of the API owner, and an
 * apiOptions object. The apiOptions can contain an API Gateway RestApi Id, a
 * region for the RestApi, and a stage that calls should be allowed/denied for.
 * For example
 *
 * new AuthPolicy(principalId,
 * AuthPolicy.PolicyDocument.getDenyAllPolicy(region, awsAccountId, restApiId,
 * stage));
 *
 * @author Jack Kohn
 */
public class AuthPolicy {
	 ...
	 * configured, the AWS Account ID that owns the RestApi, the RestApi
	 * identifier and the Stage on the RestApi that the Policy will apply to
	 */
	public static class PolicyDocument {
		...
		/**
		 * Creates a new PolicyDocument with the given context, and initializes
		 * two base Statement objects for allowing and denying access to API
		 * Gateway methods
		 *
		 * @param region
		...
		private void addResourceToStatement(Statement statement, HttpMethod httpMethod, String resourcePath) {
			...
			// to specify the root resource only, resourcePath should be an
			// empty string
		...
		/**
		 * Generates a new PolicyDocument with a single statement that allows
		 * the requested method/resourcePath
		 *
		 * @param region
		 *            API Gateway region
		 * @param awsAccountId
		 *            AWS Account that owns the API Gateway RestApi
		 * @param restApiId
		 *            RestApi identifier
		 * @param stage
		 *            Stage name
		 ...
		 *            HttpMethod to allow
		 * @param resourcePath
		 *            Resource path to allow
		 * @return new PolicyDocument that allows the requested
		 *         method/resourcePath
		 */
		public static PolicyDocument getAllowOnePolicy(String region, String awsAccountId, String restApiId,
			...
			AuthPolicy.PolicyDocument policyDocument = new AuthPolicy.PolicyDocument(region, awsAccountId, restApiId,
					stage);
		...
		/**
		 * Generates a new PolicyDocument with a single statement that denies
		 * the requested method/resourcePath
		 *
		 * @param region
		 *            API Gateway region
		 ...
		 *            HttpMethod to deny
		 * @param resourcePath
		 *            Resource path to deny
		 * @return new PolicyDocument that denies the requested
		 *         method/resourcePath
		 */
		...
		public static AuthPolicy.PolicyDocument getAllowAllPolicy(String region, String awsAccountId, String restApiId,
				String stage) {
			return getAllowOnePolicy(region, awsAccountId, restApiId, stage, HttpMethod.ALL, "*");
		}

		public static PolicyDocument getDenyAllPolicy(String region, String awsAccountId, String restApiId,
				String stage) {
	...
	static class Statement {
		...
		public static Statement getEmptyInvokeStatement(String effect) {
			return new Statement(effect, "execute-api:Invoke", new ArrayList<String>(),
